...
namespace BfTreeInterop.test
{
    ...
    public class BfTreeInteropTests
    {
        ...
        public void ScanWithCount_StartKeyInMiddle()
        {
            ...
            var results = tree.ScanWithCount(
        ...
        public void ScanWithEndKey_InclusiveRange()
        {
            ...
            var results = tree.ScanWithEndKey(
        ...
        public void ScanWithCallback_ZeroAlloc()
        {
            ...
            int count = tree.ScanWithCount("key:"u8, 100, scanBuf,
                (key, value) =>
                {
        ...
        public void SnapshotAndRecover_RoundTrip()
        {
            ...
            {
                tree.Dispose();
                tree = new BfTreeService(filePath: treePath, snapshotFilePath: snapshotPath, cbMinRecordSize: 4);
                InsertTestData(20);
                tree.CprSnapshot();
                tree.Dispose();

                // Recover from the snapshot file
                tree = BfTreeService.RecoverFromCprSnapshot(snapshotPath, null, StorageBackendType.Disk);
        ...
        public void RecoverNonExistentFile_Throws()
        {
            ...
            Assert.Throws<InvalidOperationException>(() => BfTreeService.RecoverFromCprSnapshot(path, null, StorageBackendType.Disk));
        }
        ...
        public void MemoryOnly_SnapshotThrows_PendingBfTreeSupport()
        {
            ...
            using var memTree = new BfTreeService(storageBackend: StorageBackendType.Memory, snapshotFilePath: snapshotPath, cbMinRecordSize: 4);
        ...
        public void MemoryOnly_RecoverThrows_PendingBfTreeSupport()
        {
            ...
            Assert.Throws<InvalidOperationException>(() => BfTreeService.RecoverFromCprSnapshot("/tmp/nonexistent.bftree", null, StorageBackendType.Memory));
        }
        ...
        public unsafe void Insert_NegativeKeyLength_ReturnsInvalidArguments()
        {
            ...
            fixed (byte* vp = valueBytes)
            {
                ...
                Assert.That(tree.Insert(key, value), Is.EqualTo(BfTreeInsertResult.InvalidArguments));
            }
        ...
        public unsafe void Read_NegativeKeyLength_ReturnsInvalidArguments()
        {
            ...
            {
                var key = PinnedSpanByte.FromPinnedPointer(kp, -1);
                var result = tree.Read(key, op, outputBuffer.Length, out var bytesWritten);
        ...
        public unsafe void Read_NegativeOutputBufferLength_ReturnsInvalidArguments()
        {
            ...
            {
                var key = PinnedSpanByte.FromPinnedPointer(kp, keyBytes.Length);
                var result = tree.Read(key, op, -1, out var bytesWritten);
                Assert.That(result, Is.EqualTo(BfTreeReadResult.InvalidArguments));
                Assert.That(bytesWritten, Is.EqualTo(0));
            }
        }
        ...
        public unsafe void Delete_NegativeKeyLength_DoesNotCorruptTree()
        {
            ...
            {
                var key = PinnedSpanByte.FromPinnedPointer(kp, -1);
                Assert.That(tree.Delete(key), Is.EqualTo(BfTreeDeleteResult.InvalidArguments));
            }
